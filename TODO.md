...
- [x] Fixed: every `:theme` used to leave another timestamped `config.toml.bak.<time>` (nine themes tried, nine backups; two in one second overwrote each other). Now one backup a session: the first save backs the file up as it was before tele-tui touched it, later saves in the session skip `BackupFile` but keep the atomic write and the read-back-or-restore, which restores from the bytes read before the edit
