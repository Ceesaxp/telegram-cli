...
- **Following links** — `gx` on a message arms its first link and shows where it *actually* goes; press again to cycle through the rest, `Enter` opens it in your browser, `Esc` drops it. The armed link is marked in the body so you can see which one you are about to follow. Only four schemes are ever handed to the platform opener — `http`, `https`, `mailto`, `tg` — and anything else is armed, named, and refused rather than silently skipped. Terminals that support OSC 8 still make links clickable; this is the keyboard route.
- **Forwarding** — `f` forwards the selected message to another chat. Type to filter the chats you already have; anything the server matches by title or `@username` is appended below them, so a stranger is reachable without one and a handle works when you know one. A confirmation names the destination and previews the message before anything is sent. Telegram's own forwarding is used rather than a copy, which is what keeps the original sender's attribution, the caption, the media semantics, and the server-side restriction on chats that forbid forwarding out
...
- **Contacts** — `c` opens the contact list into the chat list's column, on the same grid: selection bar, sigil, name, `@username`, and an online dot in the column the chat list gives the time. `/` narrows it over both the name and the username; `Enter` opens a private chat
...
- **Theming** — Dark and light themes with 256-color support, plus themes you write as TOML files ([Configuration](configuration.md#themes)). `:theme` switches while running and saves the choice by editing only the `theme` line of `config.toml`, with a backup; `:reload-config` applies a theme you are editing without a restart
...
- **Unread mentions** — a chat where someone mentioned you shows an `@`
  ...
  chat view jumps to the oldest unread mention, saying how many are left,
  and landing on it clears it; press it again for the next, and `ctrl+o`
  comes back. A mention further back than the jump pages is left unread
  and skipped by the next `g@`.
