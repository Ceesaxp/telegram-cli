...
| `:theme [name]` | Switch the colour theme on the spot and save it as `ui.theme`. With no name it says which theme is on. After `:theme ` the palette lists the themes — `dark` and `light` built in, then every `themes/*.toml` it can find, with its directory — opening on the one in use, marked "current", so `Enter` straight away changes nothing. Saving rewrites only the `theme` line of `config.toml` and keeps a backup; see [Configuration](configuration.md#themes). A name that is not a usable theme changes nothing and says why |
